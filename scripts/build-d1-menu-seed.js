...
categorySlugs.forEach((slug, index) => {
  ...
  lines.push(`INSERT INTO categories (slug, title_bg, title_en, title_fr, title_it, title_es, title_el, sort_order, is_active)
VALUES (${sql(slug)}, ${sql(bg)}, ${sql(en)}, ${sql(fr)}, ${sql(it)}, ${sql(es)}, ${sql(el)}, ${index + 1}, 1)
